...
/** Represents a formattable message. Such messages can use elements such as colors, formatting codes, hover and click data, and other
 * features provided by the vanilla Minecraft <a href="http://minecraft.gamepedia.com/Tellraw#Raw_JSON_Text">JSON message formatter</a>.
 ...
 * <p>
 * This class follows the builder pattern, allowing for method chaining. It is set up such that invocations of property-setting methods will
 * affect the current editing component, and a call to {@link #then()} or {@link #then(String)} will append a new editing component to the
 * end of the message, optionally initializing it with text. Further property-setting method calls will affect that editing component.
 * </p>
 */
public class FancyMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable {
	...
	/** Set the behavior of the current editing component to instruct the client to open a file on the client side filesystem when the
	...
	/** Set the behavior of the current editing component to instruct the client to open a webpage in the client's web browser when the
	 * currently edited part of the {@code FancyMessage} is clicked.
	 * @param url
	...
	/** Set the behavior of the current editing component to instruct the client to replace the chat input box content with the specified
	 * string when the currently edited part of the {@code FancyMessage} is clicked. The client will not immediately send the command to the
	 * server to be executed unless the client player submits the command/chat message, usually with the enter key.
	 * @param command
	...
	/** Set the behavior of the current editing component to instruct the client to append the chat input box content with the specified
	 * string when the currently edited part of the {@code FancyMessage} is SHIFT-CLICKED. The client will not immediately send the command
	 * to the server to be executed unless the client player submits the command/chat message, usually with the enter key.
	 * @param command
	...
	/** Set the behavior of the current editing component to instruct the client to send the specified string to the server as a chat
	 * message when the currently edited part of the {@code FancyMessage} is clicked. The client <b>will</b> immediately send the command to
	 * the server to be executed when the editing component is clicked.
	 * @param command
	 *            The text to display in the chat bar of the client.
	 * @return This builder instance. */
	 ...
	 * </p>
	 * @param lines
	 *            The lines of text which will be displayed to the client upon hovering. The iteration order of this object will be the
	 *            order in which the lines of the tooltip are created.
	 * @return This builder instance. */
	...
	/** Set the behavior of the current editing component to display the specified lines of formatted text when the client hovers over the
	 * text.
	 * <p>
	 * Tooltips do not inherit display characteristics, such as color and styles, from the message component on which they are applied.
	 * </p>
	...
	/** If the text is a translatable key, and it has replaceable values, this function can be used to set the replacements that will be
	 * used in the message.
	 * @param replacements
	...
	/** Terminate construction of the current editing component, and begin construction of a new message component. After a successful call
	 * to this method, all setter methods will refer to a new message component, created as a result of the call to this method.
	 * @param text
	...
	/** Serialize this fancy message, converting it into syntactically-valid JSON using a {@link JsonWriter}. This JSON should be compatible
	 * with vanilla formatter commands such as {@code /tellraw}.
	...
	/** Sends this message to a command sender. If the sender is a player, they will receive the fully-fledged formatted display of this
	 * message. Otherwise, they will receive a version of this message with less formatting.
	 * @param sender
	...
	/** Convert this message to a human-readable string with limited formatting. This method is used to send this message to clients without
	...
	/** Deserializes a JSON-represented message from a mapping of key-value pairs. This is called by the Bukkit serialization API. It is not
	 * intended for direct public API consumption.
	 * @param serialized
		...
		for (JsonElement mPrt : extra) {
			...
			for (Map.Entry<String, JsonElement> entry : messagePart.entrySet()) {
				...
				if (TextualComponent.isTextKey(entry.getKey())) {
					...
					Map<String, Object> serializedMapForm = new HashMap<String, Object>(); // Must be object due to Bukkit serializer API
																							// compliance
					...
					if (object.get("value").isJsonPrimitive()) {
						...
						component.hoverActionData = deserialize(
								object.get("value").toString() /* This should properly serialize the JSON object as a JSON string */);
					}
